...
{
	struct bd7181x_power *pwr = psy->drv_data;
	u32 vot;
					   ...
					   size_t count)
{
	...
	struct bd7181x_power *pwr = power_supply_get_drvdata(psy);
	ssize_t ret = 0;
	unsigned int reg;
	unsigned int val;
...
static int __init bd7181x_power_probe(struct platform_device *pdev)
{
	...
	pwr->bat = power_supply_register(&pdev->dev, &pwr->bat_desc, &bat_cfg);
	if (IS_ERR(pwr->bat)) {
	...
	bat_cfg.supplied_to = bd7181x_ac_supplied_to;
	bat_cfg.num_supplicants = ARRAY_SIZE(bd7181x_ac_supplied_to);

	pwr->ac = power_supply_register(&pdev->dev, &pwr->ac_desc, &bat_cfg);
	if (IS_ERR(pwr->ac)) {
